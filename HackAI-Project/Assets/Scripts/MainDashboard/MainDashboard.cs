    ...
    void Awake()
    {
        ...
        foreach (GameData dat in profileManager.playerProfile.gameHistory) {
            switch(dat.name) {
                ...
                case "BreathSync":
                    graphData.Add(new DataElement{
                        amount = 1f - Mathf.Round((dat.data[0] - profileManager.playerProfile.personalBests[1]) / 
                                             (profileManager.playerProfile.personalBests[1] - profileManager.playerProfile.personalLows[1])
                                            * 10f) / 10f, 
        ...
        for(int i = 0; i < 5; i++) {
            moodIndicators[i].GetChild(0).GetComponent<TextMeshProUGUI>().text = $"- {profileManager.playerProfile.moodData[i]}";
        }
